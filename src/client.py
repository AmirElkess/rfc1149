...
ip = "192.168.1.13" #Of the server (Change to your pcs address)
tcp_port = 65432
udp_port = 2021
...
def client_server_loop(message):
    ...
    # 1) send(msg)      --------------> recv(msg)
    # 2) recv(msg)      <-------------- send(msg)
    UDPClientSocket.sendto(str.encode(message), (ip, udp_port))
    try:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as TCPClientSocket:
        ...
        while True:
            ...
            for i in range(n):
                ...
                message = message[::-1]
                message = message[:s]
                TCPClientSocket.sendall(message.encode('ascii'))
...
def draw():
    ...
    ani = FuncAnimation(plt.gcf(), live_draw, interval=250)
    plt.show()
